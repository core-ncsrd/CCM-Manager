...
logger.info("[%s]: #--------------------------------------------------#", main_process_id)
logger.info("[%s]: #----------- INITIATING LOCAL CCM AGENT -----------#", main_process_id)
logger.info("[%s]: #--------------------------------------------------#", main_process_id)
...
def populate_json():
    ...
    logger.info("[%s]: Populate oid_mappings.json file with system's hardwired OID information.", main_process_id)
    ...
    logger.info("[%s]: Parse security levels.", main_process_id)
    custom_id = 4180
    ...
    logger.info("[%s]: Load OID mappings JSON file and populate algorithms-oid block.", main_process_id)
    with open(oid_mappings_file, "r") as json_file:
        oids = json.load(json_file)

    # Parse the algorithms file for security levels
    logger.info("[%s]: Parse the algorithms file to get the security levels.", main_process_id)
    custom_id = 4180
...
#    print("Algorithms security levels", algorithm_security_levels)
    ...
    logger.info("[%s]: Call upon a function to get the list of the system's disabled algorithms.", main_process_id)
    custom_id = 8446
...
#    print("Disabled Algos: ", disabled_algorithms)
    ...
    logger.info("[%s]: Filter out disabled algorithms from the list the CCM Agent sends to the CCM Manager.", main_process_id)
    custom_id = 8446
    ...
    logger.info("[%s]: Load the OID mappings JSON file for update.", main_process_id)
    with open("oid_mappings.json", "r") as json_file:
        algorithms = json.load(json_file)

    # Update algorithms with security levels
    logger.info("[%s]: update the algorithms with their respective security level.", main_process_id)
    ...
    logger.info("[%s]: Call upon function to gather SSH crypto information (ciphers, MACs, KEX, etc).", main_process_id)
    custom_id = 4253
...
#    ssh_crypto_info, algorithms = filter_matching_algorithms(ssh_crypto_info, algorithms)
    ...
    logger.info("[%s]: Call upon function to gather TLS cipher details by executing the command \"openssl ciphers -v\".", main_process_id)
    custom_id = 6101
    ...
    logger.info("[%s]: Upate cipher data gathered with their respective security levels.", main_process_id)
    ...
    logger.info("[%s]: Call upon function to gather nmap TLS information from the host.", main_process_id)
    custom_id = 6101
    nmap_tls_dets = get_nmap_tls_info()
    custom_id = 22524368

    # Get certificate information by calling on the function get_certificate_info()
    logger.info("[%s]: Call upon function to gather certificate centric information of the host.", main_process_id)
    custom_id = 5280
    ...
    logger.info("[%s]: Construct data structure to collect cryptographic related information to send to the CCM Manager API.", main_process_id)
    data = {
        "oid_refs": algos,
        "ciphers": {"tls" : ciphers},
...
#        "kernel_crypto_info": kernels ## LATER
    ...
    logger.info("[%s]: Append data structure to the final file.", main_process_id)
    with open(output_file, "w") as json_file:
        json.dump(data, json_file, indent=2)

def send_json_to_api(file_path, api_url):
    logger.info("[%s]: Trying to send file %s to the CCM Manager..........", main_process_id, file_path)
    if not os.path.exists(file_path):
        logger.error("[%s]: Error: file %s does not exist.", main_process_id, file_path)
        return None
    ...
    try:
        ...
        logger.info("[%s]: File %s successfully sent to %s. response: %s", main_process_id, file_path, api_url, response.text)
        return response.json()
    except requests.exceptions.RequestException as e:
        logger.error("[%s]: Error sending file %s to CCM Manager API %s: %s", main_process_id, file_path, api_url, e)
        return None
...
if __name__ == "__main__":
    ...
    populate_json()
    logger.info("[%s]: Data appended to %s.", main_process_id, output_file)

    logger.info("[%s]: Sending %s to CCM Manager for further proscessing...", main_process_id, output_file)
    logger.info("[%s]: Trying to connect with CCM Manager...", main_process_id)
    ...
    #
    # if response is None:
    #     logger.error("[%s]: Error sending %s to API.", main_process_id, output_file)
    # elif isinstance(response, dict) and "error" in response:
    #     logger.error("[%s]: Error sending %s to API: %s", main_process_id, output_file, response)
    # else:
    #     logger.info("[%s]: %s successfully sent to CCM Manager. API response: %s", main_process_id, output_file, response)
    ...
    logger.info("[%s]: *** Total time elapsed: %s minutes %s seconds ***", main_process_id, minutes, seconds)
    print(f"*** Total time elapsed: {minutes} minutes {seconds} seconds ***", main_process_id)
    logger.info("[%s]: #--------------------------------------------------#", main_process_id)
    logger.info("[%s]: #----------- TERMINATING LOCAL CCM AGENT ----------#", main_process_id)
    logger.info("[%s]: #--------------------------------------------------#", main_process_id)
